...
import static com.social.instagram.util.httpstatus.ResponseConstants.RESPONSE_AWS_S3_FILE_FAIL_ACCESS;
...
public class ApiExceptionHandler {
    ...
    */
    ...
    public ResponseEntity<String> handleDuplicateUserIdException(UserIdDuplicatedException exception) {
        log.error("중복된 아이디가 존재합니다", exception);
    ...
    public ResponseEntity<String> handleNotUserDataException(UserNotAccountException exception) {
        log.error("입력된 이메일과 일치하는 계정이 없습니다", exception);
    ...
    public ResponseEntity<String> handleMethodArgumentNotValidException(MethodArgumentNotValidException exception) {
        ...
        log.error(errorMessage.getErrorMessage(), exception);
    ...
    public ResponseEntity<String> handleUserNotLoginException(UserNotLoginException exception) {
        log.error("유저에 대한 정보를 찾지 못했습니다. 로그인을 해주세요", exception);
    ...
    @ExceptionHandler(AwsS3FileAccessFailedException.class)
    public ResponseEntity<String> handleAwsS3FileAccessFailedException(AwsS3FileAccessFailedException exception) {
        log.error("S3 업로드 작업시 파일에 대한 엑세스가 실패 했습니다", exception);
        return RESPONSE_AWS_S3_FILE_FAIL_ACCESS;
    }
